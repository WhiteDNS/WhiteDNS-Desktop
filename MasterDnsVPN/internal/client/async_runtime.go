...
func (c *Client) StartAsyncRuntime(parentCtx context.Context) error {
	...
	c.startPostConnectStartupMTULossVerification(runtimeCtx)
...
func (c *Client) asyncPlanEncodeWorker(ctx context.Context, id int) {
	...
	for {
		...
		case task, ok := <-c.plannerQueue:
			...
			conns, err = c.balancer.SelectTargetsForPayload(task.opts.PacketType, task.opts.StreamID, targetCount, len(task.opts.Payload))
			if err != nil {
				conns = nil
			}
			...
			if len(conns) == 0 {
				c.debugf(
					"planner has no resolver targets: packet=%s stream=%d payload=%d dup=%d active=%d",
					Enums.PacketTypeName(task.opts.PacketType),
					task.opts.StreamID,
					len(task.opts.Payload),
					targetCount,
			...
			if task.selected != nil {
				if groupMTU := c.effectiveGroupUploadMTU(conns); groupMTU > 0 {
					if task.opts.PacketType != Enums.PACKET_STREAM_DATA && task.opts.PacketType != Enums.PACKET_STREAM_RESEND {
						c.debugf(
							"planner hot-applied stream upload MTU: stream=%d packet=%s group_mtu=%d payload=%d %s",
							task.opts.StreamID,
							Enums.PacketTypeName(task.opts.PacketType),
							groupMTU,
							len(task.opts.Payload),
					...
					task.selected.SetUploadMTU(groupMTU)
				}
			}
