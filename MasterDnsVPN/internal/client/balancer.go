...
const balancerMTUCompatibilityPercent = 80
...
func (b *Balancer) SeedResolverStats(serverKey string, sent int, lost int, rttSum time.Duration, rttCount int) {
	stats := b.statsForKey(serverKey)
	if stats == nil || sent <= 0 {
		return
	}
	...
	stats.acked.Add(uint64(acked))
	stats.lost.Add(uint64(lost))
	if rttSum > 0 && rttCount > 0 {
		stats.rttMicrosSum.Add(uint64(rttSum / time.Microsecond))
		stats.rttCount.Add(uint64(rttCount))
	}
	stats.applyHalfLife()
}
...
func (b *Balancer) ReportTimeout(serverKey string, now time.Time, window time.Duration, minActive int) bool {
	...
	thresholdResolver := ""
	thresholdDomain := ""
	thresholdMinObservations := 0
	thresholdActiveCount := 0
	thresholdLogged := false
	logThreshold := func() {
		if thresholdLogged || thresholdResolver == "" {
			return
		}
		thresholdLogged = true
		b.debugf(
			"resolver auto-disable threshold reached: key=%s resolver=%s domain=%s timed_out=%d sent=%d min_observations=%d active=%d window=%s",
			serverKey,
			thresholdResolver,
			thresholdDomain,
			totalTimedOut,
			totalSent,
			thresholdMinObservations,
			thresholdActiveCount,
			window,
		)
	}
	...
	thresholdResolver = conn.ResolverLabel
	thresholdDomain = conn.Domain
	thresholdMinObservations = minObservations
	thresholdActiveCount = activeCount
	...
	if activeCount <= minActive {
		...
		logThreshold()
		b.debugf(
			"resolver auto-disable skipped by min-active guard: key=%s resolver=%s active=%d min_active=%d",
			serverKey,
			resolverLabel,
			activeCount,
			minActive,
	...
	if confirmRequired {
		...
		if !down {
			...
			b.mu.Unlock()
			b.debugf(
				"resolver auto-disable skipped by confirmation probe: key=%s resolver=%s active=%d window=%s",
				serverKey,
				resolverLabel,
				activeCount,
				window,
	...
	connCopy := *conn
	remainingActive := len(b.activeIDs)
	handler := b.onResolverDisabled
	b.mu.Unlock()

	logThreshold()
	if handler != nil {
		handler(&connCopy, "TIMEOUT")
	}
	if b.log != nil {
		b.log.Warnf("<red>DNS Resolver disabled (100%% Loss): <cyan>%s</cyan> <cyan>%s</cyan>) | <cyan>%s</cyan> | Remaining: <cyan>%d</cyan></red>",
			connCopy.ResolverLabel, connCopy.Domain, connCopy.Resolver, remainingActive)
	}
	b.debugf(
		"resolver auto-disable applied: key=%s resolver=%s domain=%s remaining_active=%d sent_window=%d lost_window=%d",
		serverKey,
		connCopy.ResolverLabel,
		connCopy.Domain,
		remainingActive,
		totalSent,
		totalTimedOut,
...
func (b *Balancer) SelectTargets(packetType uint8, streamID uint16, requiredCount int) ([]Connection, error) {
	return b.SelectTargetsForPayload(packetType, streamID, requiredCount, 0)
}

func (b *Balancer) SelectTargetsForPayload(packetType uint8, streamID uint16, requiredCount int, minUploadMTU int) ([]Connection, error) {
	b.mu.RLock()
	defer b.mu.RUnlock()
	...
	if requiredCount == 1 || streamID == 0 || !isBalancerStreamDataLike(packetType) {
		selected := b.getUniqueConnectionsForUploadLocked(requiredCount, minUploadMTU)
		if len(selected) == 0 {
			return nil, ErrNoValidConnections
		}
		return selected, nil
	...
	if state.PreferredResolverKey == "" && b.strategy == BalancingLeastLoss && requiredCount > 1 && b.hasLossSignalLocked() {
		if selected, ok := b.selectLeastLossMTUCompatibleGroupLocked(requiredCount, "", minUploadMTU); ok {
			state.PreferredResolverKey = selected[0].Key
			state.ResendStreak = 0
			b.debugf(
				"least-loss MTU group selected for new stream: stream=%d packet=%s required=%d min_upload=%d %s",
				streamID,
				Enums.PacketTypeName(packetType),
				requiredCount,
				minUploadMTU,
				debugConnectionSummary(selected, requiredCount),
			)
			return selected, nil
		}
		b.debugf(
			"least-loss MTU group unavailable for new stream; falling back: stream=%d packet=%s required=%d min_upload=%d active=%d",
			streamID,
			Enums.PacketTypeName(packetType),
			requiredCount,
			minUploadMTU,
	...
	if minUploadMTU > 0 && preferred.UploadMTUBytes > 0 && preferred.UploadMTUBytes < minUploadMTU {
		if selected := b.getUniqueConnectionsForUploadLocked(requiredCount, minUploadMTU); len(selected) >= requiredCount {
			b.debugf(
				"sticky preferred resolver below payload MTU; selected alternate group: stream=%d preferred=%s preferred_up=%d min_upload=%d %s",
				streamID,
				preferred.ResolverLabel,
				preferred.UploadMTUBytes,
				minUploadMTU,
				debugConnectionSummary(selected, requiredCount),
			)
			return selected, nil
	...
	if b.strategy == BalancingLeastLoss && requiredCount > 1 && b.hasLossSignalLocked() {
		if selected, ok := b.selectLeastLossMTUCompatibleGroupLocked(requiredCount, preferred.Key, minUploadMTU); ok {
			if packetType != Enums.PACKET_STREAM_DATA && packetType != Enums.PACKET_STREAM_RESEND {
				b.debugf(
					"least-loss MTU group selected with sticky anchor: stream=%d anchor=%s required=%d min_upload=%d %s",
					streamID,
					preferred.ResolverLabel,
					requiredCount,
					minUploadMTU,
	...
	if remaining := requiredCount - 1; remaining > 0 {
		others := b.getUniqueConnectionsExcludingForUploadLocked(remaining, preferred.Key, minUploadMTU)
		selected = append(selected, others...)
	}
	if requiredCount > 1 && len(selected) < requiredCount {
		b.debugf(
			"resolver duplication underfilled: stream=%d packet=%s required=%d selected=%d min_upload=%d",
			streamID,
			Enums.PacketTypeName(packetType),
			requiredCount,
			len(selected),
...
func (b *Balancer) clearPreferredResolverReferencesLocked(serverKey string) {
	...
	for _, state := range b.streamRoutes {
		...
		state.mu.Lock()
		if state.PreferredResolverKey != serverKey {
			state.mu.Unlock()
			continue
		}
...
func (b *Balancer) getUniqueConnectionsExcludingLocked(requiredCount int, excludeKey string) []Connection {
	return b.getUniqueConnectionsExcludingForUploadLocked(requiredCount, excludeKey, 0)
}

func (b *Balancer) getUniqueConnectionsExcludingForUploadLocked(requiredCount int, excludeKey string, minUploadMTU int) []Connection {
	if requiredCount <= 0 || len(b.activeIDs) == 0 {
		return nil
	}

	all := b.getUniqueConnectionsForUploadLocked(requiredCount+1, minUploadMTU)
	selected := make([]Connection, 0, requiredCount)
...
func (b *Balancer) getUniqueConnectionsLocked(requiredCount int) []Connection {
	return b.getUniqueConnectionsForUploadLocked(requiredCount, 0)
}

func (b *Balancer) getUniqueConnectionsForUploadLocked(requiredCount int, minUploadMTU int) []Connection {
	...
	default:
		if b.strategy == BalancingLeastLoss && count > 1 && b.hasLossSignalLocked() {
			if selected, ok := b.selectLeastLossMTUCompatibleGroupLocked(count, "", minUploadMTU); ok {
				return selected
			}
			if minUploadMTU > 0 {
				if selected := b.selectLeastLossAboveUploadLocked(count, minUploadMTU, ""); len(selected) >= count {
					return selected
				}
			}
		}
...
type mtuLossCandidate struct {
	idx  int
	loss uint64
}

func (b *Balancer) selectLeastLossMTUCompatibleGroupLocked(count int, anchorKey string, minUploadMTU int) ([]Connection, bool) {
	if count <= 1 || len(b.activeIDs) == 0 {
		return nil, false
	}
	candidates := b.leastLossMTUCandidatesLocked(minUploadMTU)
	...
	if anchorKey != "" {
		...
		for _, candidate := range candidates {
			if b.connections[candidate.idx].Key == anchorKey {
				anchorIdx = candidate.idx
				break
			}
		}
		if anchorIdx < 0 {
		...
		group, ok := b.buildMTUCompatibleGroupLocked(anchorIdx, candidates, count)
		if !ok {
			return nil, false
		}
		return b.connectionsByIndicesLocked(group), true
	}

	for _, candidate := range candidates {
		group, ok := b.buildMTUCompatibleGroupLocked(candidate.idx, candidates, count)
		if !ok {
			continue
		}
		return b.connectionsByIndicesLocked(group), true
...
func (b *Balancer) leastLossMTUCandidatesLocked(minUploadMTU int) []mtuLossCandidate {
	candidates := make([]mtuLossCandidate, 0, len(b.activeIDs))
	for _, idx := range b.activeIDs {
		...
		if minUploadMTU > 0 && conn.UploadMTUBytes < minUploadMTU {
			continue
		}
		candidates = append(candidates, mtuLossCandidate{
			idx:  idx,
...
func (b *Balancer) selectLeastLossAboveUploadLocked(count int, minUploadMTU int, excludeKey string) []Connection {
	if count <= 0 || minUploadMTU <= 0 {
		return nil
	}
	candidates := b.leastLossMTUCandidatesLocked(minUploadMTU)
	if len(candidates) == 0 {
	...
	for _, candidate := range candidates {
		if excludeKey != "" && b.connections[candidate.idx].Key == excludeKey {
			continue
		}
		selected = append(selected, candidate.idx)
		if len(selected) >= count {
			return b.connectionsByIndicesLocked(selected)
		}
	}
	return nil
}

func (b *Balancer) buildMTUCompatibleGroupLocked(anchorIdx int, candidates []mtuLossCandidate, count int) ([]int, bool) {
	if anchorIdx < 0 || anchorIdx >= len(b.connections) {
		return nil, false
	}
	anchor := b.connections[anchorIdx]
	if !anchor.IsValid || anchor.UploadMTUBytes <= 0 {
		return nil, false
	}

	group := make([]int, 0, count)
	group = append(group, anchorIdx)
	minMTU := anchor.UploadMTUBytes
	maxMTU := anchor.UploadMTUBytes

	for _, candidate := range candidates {
		if candidate.idx == anchorIdx {
			continue
		}
		conn := b.connections[candidate.idx]
		if !conn.IsValid || conn.UploadMTUBytes <= 0 {
			continue
		}
		nextMin := min(minMTU, conn.UploadMTUBytes)
		nextMax := max(maxMTU, conn.UploadMTUBytes)
		if !mtuValuesCompatible(nextMin, nextMax) {
			continue
		}
		group = append(group, candidate.idx)
		minMTU = nextMin
		maxMTU = nextMax
		if len(group) >= count {
			return group, true
...
func mtuValuesCompatible(minMTU int, maxMTU int) bool {
	if minMTU <= 0 || maxMTU <= 0 {
		return false
	}
	return minMTU*100 >= maxMTU*balancerMTUCompatibilityPercent
}
