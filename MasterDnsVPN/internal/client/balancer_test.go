...
func TestBalancerLeastLossDuplicateSelectionPrefersCompatibleMTUBucket(t *testing.T) {
	b := NewBalancer(BalancingLeastLoss, nil)
	...
	b.SeedResolverStats("c", 10, 1, 10*time.Millisecond, 10)

	selected, err := b.SelectTargets(Enums.PACKET_PING, 0, 2)
	if err != nil {
		t.Fatal(err)
	}
	if len(selected) != 2 || selected[0].Key != "b" || selected[1].Key != "c" {
		t.Fatalf("expected compatible lower-MTU bucket b,c, got %#v", selected)
	}
}

func TestBalancerLeastLossDuplicateSelectionKeepsStickyPreferredAndCompatibleCompanion(t *testing.T) {
	b := NewBalancer(BalancingLeastLoss, nil)
	b.SetConnections([]*Connection{
		{Key: "a"},
		{Key: "b"},
	...
	setBalancerMTUActive(b, "a", 100)
	setBalancerMTUActive(b, "b", 95)
	setBalancerMTUActive(b, "c", 50)
	b.SeedResolverStats("a", 10, 0, 10*time.Millisecond, 10)
	b.SeedResolverStats("b", 10, 2, 10*time.Millisecond, 10)
	b.SeedResolverStats("c", 10, 0, 10*time.Millisecond, 10)

	b.EnsureStream(7)
	b.mu.Lock()
	b.streamRoutes[7].PreferredResolverKey = "a"
	b.mu.Unlock()

	selected, err := b.SelectTargets(Enums.PACKET_STREAM_DATA, 7, 2)
	if err != nil {
		t.Fatal(err)
	}
	if len(selected) != 2 || selected[0].Key != "a" || selected[1].Key != "b" {
		t.Fatalf("expected sticky preferred a with compatible b, got %#v", selected)
	}
}

func TestBalancerLeastLossDuplicateSelectionFallsBackWhenNoCompatibleBucket(t *testing.T) {
	b := NewBalancer(BalancingLeastLoss, nil)
	b.SetConnections([]*Connection{
		{Key: "a"},
		{Key: "b"},
	...
	setBalancerMTUActive(b, "a", 140)
	setBalancerMTUActive(b, "b", 70)
	setBalancerMTUActive(b, "c", 40)
	b.SeedResolverStats("a", 10, 0, 10*time.Millisecond, 10)
	b.SeedResolverStats("b", 10, 1, 10*time.Millisecond, 10)
	...
	if len(selected) != 2 || selected[0].Key != "a" || selected[1].Key != "b" {
		t.Fatalf("expected least-loss fallback a,b, got %#v", selected)
	}
}

func TestBalancerLeastLossDuplicateSelectionWithMissingMTUPreservesCurrentBehavior(t *testing.T) {
	b := NewBalancer(BalancingLeastLoss, nil)
	b.SetConnections([]*Connection{
		{Key: "a"},
		{Key: "b"},
	...
	if len(selected) != 2 || selected[0].Key != "a" || selected[1].Key != "b" {
		t.Fatalf("expected original least-loss selection a,b, got %#v", selected)
	}
}

func TestBalancerLeastLossPayloadAwareSelectionAvoidsOversizedLowMTUTarget(t *testing.T) {
	b := NewBalancer(BalancingLeastLoss, nil)
	b.SetConnections([]*Connection{
		{Key: "a"},
		{Key: "b"},
	...
	setBalancerMTUActive(b, "a", 140)
	setBalancerMTUActive(b, "b", 70)
	setBalancerMTUActive(b, "c", 138)
	b.SeedResolverStats("a", 10, 0, 10*time.Millisecond, 10)
	b.SeedResolverStats("b", 10, 1, 10*time.Millisecond, 10)
	b.SeedResolverStats("c", 10, 2, 10*time.Millisecond, 10)

	selected, err := b.SelectTargetsForPayload(Enums.PACKET_PING, 0, 2, 130)
	if err != nil {
		t.Fatal(err)
	}
	if len(selected) != 2 || selected[0].Key != "a" || selected[1].Key != "c" {
		t.Fatalf("expected payload-safe selection a,c, got %#v", selected)
	}
}
...
func setBalancerMTUActive(b *Balancer, key string, uploadMTU int) {
	_ = b.ApplyMTUProbeResult(key, uploadMTU, uploadMTU, 300, 0, true)
}
