...
type Client struct {
	...
	resolverClassMu   sync.RWMutex
	resolverClass     map[string]resolverClassification
	nowFn             func() time.Time

	// MTU States
	mtuStateMu                            sync.RWMutex
	syncedUploadMTU                       int
	...
	fullInitialMTUScan                    bool
	skipInitialMTUScan                    bool
	mtuProbeFunc                          func(context.Context, Connection, int) (mtuConnectionProbeResult, mtuRejectReason)
	mtuStartupLossProbeFunc               func(context.Context, Connection, mtuProbeAxis, int, int, int, time.Duration) mtuLossProbeStats
	mtuRecheckOnce                        sync.Once
	mtuStartupLossRunning                 atomic.Bool
	mtuStartupLossDone                    atomic.Bool
	scannerReportPath                     string
	scannerInputPath                      string
	scannerReportActive                   atomic.Bool
	scannerReportMu                       sync.Mutex
	scannerReportSeen                     map[string]struct{}
...
func New(cfg config.ClientConfig, log *logger.Logger, codec *security.Codec) *Client {
	...
	c := &Client{
		...
		resolverClass:                         make(map[string]resolverClassification),
		...
		mtuScanControlFile:                    os.Getenv(mtuScanControlEnv),
		fullInitialMTUScan:                    whitednsBoolEnv(fullInitialMTUScanEnv),
		skipInitialMTUScan:                    whitednsBoolEnv(skipInitialMTUScanEnv),
		scannerReportPath:                     os.Getenv(scannerReportEnv),
		scannerInputPath:                      os.Getenv(scannerInputEnv),
		scannerReportSeen:                     make(map[string]struct{}),
		lastMTUProgressPercent:                -1,
	...
	c.debugf(
		"client config loaded: resolvers=%d domains=%d strategy=%d packet_dup=%d setup_dup=%d mtu_up=%d..%d mtu_down=%d..%d mtu_retries=%d mtu_timeout=%.3gs mtu_parallel=%d full_initial_mtu_scan=%t skip_initial_mtu_scan=%t startup_loss_enabled=%t startup_loss_samples=%d startup_loss_max=%d startup_loss_candidates=%d mtu_recheck_enabled=%t mtu_recheck_interval=%dm auto_disable=%t auto_disable_window=%.3gs workers=%d process_workers=%d rx_channel=%d socks_udp_timeout=%.3gs ping_intervals=%.3g/%.3g/%.3g/%.3g",
		len(cfg.Resolvers),
		len(cfg.Domains),
		...
		cfg.MTUTestRetries,
		cfg.MTUTestTimeout,
		cfg.EffectiveMTUTestParallelism(),
		c.fullInitialMTUScan,
		c.skipInitialMTUScan,
		cfg.MTUStartupLossVerifyEnabled,
		cfg.MTUStartupLossVerifySamples,
		cfg.MTUStartupLossVerifyMaxLossPercent,
		cfg.MTUStartupLossVerifyCandidates,
		cfg.MTURecheckEnabled,
		cfg.MTURecheckIntervalMinutes,
		cfg.AutoDisableTimeoutServers,
		cfg.AutoDisableTimeoutWindowSeconds,
		cfg.RX_TX_Workers,
		cfg.TunnelProcessWorkers,
		cfg.EffectiveRXChannelSize(),
		cfg.SOCKSUDPAssociateReadTimeoutSeconds,
		cfg.PingAggressiveIntervalSeconds,
		cfg.PingLazyIntervalSeconds,
		cfg.PingCooldownIntervalSeconds,
		cfg.PingColdIntervalSeconds,
	)
	return c
}
	...
	for {
		...
		default:
			if !c.successMTUChecks {
				c.debugf(
					"entering initial MTU stage: total_resolvers=%d active=%d full_initial_mtu_scan=%t skip_initial_mtu_scan=%t",
					c.balancer.TotalCount(),
					len(c.balancer.ActiveConnections()),
					c.fullInitialMTUScan,
					c.skipInitialMTUScan,
				)
				if err := c.runInitialMTUStage(ctx); err != nil {
				...
				if mtuState.UploadMTU <= 0 || mtuState.DownloadMTU <= 0 {
					c.successMTUChecks = false
					c.log.Errorf("<red>❌ MTU tests failed: Upload MTU: %d, Download MTU: %d</red>", mtuState.UploadMTU, mtuState.DownloadMTU)
					select {
					case <-ctx.Done():
						c.notifySessionCloseBurst(time.Second)
						c.StopAsyncRuntime()
				...
				c.debugf(
					"initial MTU stage complete: synced_upload=%d synced_download=%d upload_chars=%d active=%d",
					mtuState.UploadMTU,
					mtuState.DownloadMTU,
					mtuState.UploadChars,
					len(c.balancer.ActiveConnections()),
				)
				c.startMTURecheckLoop(ctx)
			...
			if !c.sessionReady {
				...
				c.debugf(
					"session initialization succeeded: session_id=%d active_resolvers=%d retries=%d racing_count=%d",
					c.sessionID,
					len(c.balancer.ActiveConnections()),
					retries,
				...
				if handled, err := c.runScannerAfterConnectIfNeeded(ctx); handled {
					return err
				}
			}
