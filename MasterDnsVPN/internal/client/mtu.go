...
func (c *Client) runInitialMTUStage(ctx context.Context) error {
...
func (c *Client) RunInitialMTUTests(ctx context.Context) error {
	...
	if c.balancer.TotalCount() == 0 {
		return ErrNoValidConnections
	}
	if c.skipInitialMTUScan {
		return c.activateTrustedInitialMTUConnections()
	}
	c.debugf(
		"initial MTU tests starting: mode=%s total_resolvers=%d packet_dup=%d setup_dup=%d mtu_parallel=%d timeout=%s",
		map[bool]string{true: "full-scan", false: "early-start"}[c.fullInitialMTUScan],
		c.balancer.TotalCount(),
		c.cfg.PacketDuplicationCount,
		c.cfg.SetupPacketDuplicationCount,
		c.cfg.EffectiveMTUTestParallelism(),
		c.mtuTestTimeout,
	)
	c.resetResolverClassificationsPending(c.balancer.AllConnections())
	if c.fullInitialMTUScan {
		return c.runFullMTUTests(ctx)
	}
	return c.runEarlyStartMTUTests(ctx)
}

func (c *Client) activateTrustedInitialMTUConnections() error {
	connections := c.balancer.AllConnections()
	if len(connections) == 0 {
		return ErrNoValidConnections
	}

	uploadMTU := trustedInitialMTU(c.cfg.MaxUploadMTU, c.cfg.MinUploadMTU)
	downloadMTU := trustedInitialMTU(c.cfg.MaxDownloadMTU, c.cfg.MinDownloadMTU)
	if uploadMTU <= 0 || downloadMTU <= 0 {
		return ErrNoValidConnections
	}
	uploadChars := c.encodedCharsForPayload(uploadMTU)
	if uploadChars <= 0 {
		uploadChars = uploadMTU
	}

	c.debugf(
		"skipping initial MTU scan with trusted resolver state: total=%d upload=%d download=%d upload_chars=%d",
		len(connections),
		uploadMTU,
		downloadMTU,
		uploadChars,
	...
	if c.log != nil {
		c.log.Infof(
			"<green>Skipping initial MTU tests with trusted resolver MTU: upload=<cyan>%d</cyan>, download=<cyan>%d</cyan></green>",
			uploadMTU,
			downloadMTU,
		)
	}

	c.resetResolverClassificationsPending(connections)
	c.prepareMTUSuccessOutputFile()
	c.logConnectionProgress("mtu", 80, "completed", len(connections), "total", len(connections), "valid", len(connections), "rejected", 0)
	...
	for _, conn := range connections {
		...
		conn.UploadMTUBytes = uploadMTU
		conn.UploadMTUChars = uploadChars
		conn.DownloadMTUBytes = downloadMTU
		conn.IsValid = true
		_ = c.balancer.ApplyMTUProbeResult(conn.Key, conn.UploadMTUBytes, conn.UploadMTUChars, conn.DownloadMTUBytes, 0, true)
	...
	c.applySyncedMTUState(uploadMTU, downloadMTU, uploadChars)
	c.appendMTUUsageSeparatorOnce()
	c.logConnectionProgress("selecting", 85, "valid", len(active))
	c.logMTUCompletion(active)
	c.logResolverRuntimeState()
	return nil
}

func trustedInitialMTU(values ...int) int {
...
func (c *Client) runFullMTUTests(ctx context.Context) error {
	...
	c.debugf("full MTU scan configured: total=%d workers=%d", len(scanConnections), workerCount)
	c.logMTUStart(workerCount)
	c.prepareMTUSuccessOutputFile()

	counters := &mtuScanCounters{}
	c.logMTUProgress(counters, len(scanConnections))
	if workerCount <= 1 {
	...
	c.logResolverRuntimeState()
	c.debugf(
		"full MTU scan complete: %s synced_upload=%d synced_download=%d upload_chars=%d",
		debugConnectionSummary(validConns, 8),
		minUpload,
		minDownload,
		minUploadChars,
	)
...
func (c *Client) runEarlyStartMTUTests(ctx context.Context) error {
	...
	counters := &mtuScanCounters{}
	c.logMTUProgress(counters, len(scanConnections))
	results := c.startMTUProbeScan(ctx, scanConnections, uploadCaps, workerCount, counters)
	target := c.initialMTUEarlyStartTarget(len(scanConnections))
	c.debugf(
		"early-start MTU scan configured: total=%d workers=%d early_target=%d packet_dup=%d setup_dup=%d",
		len(scanConnections),
		workerCount,
		target,
		c.cfg.PacketDuplicationCount,
	...
	for result := range results {
		if result.conn.IsValid {
			...
			c.debugf(
				"early-start MTU valid resolver: resolver=%s domain=%s up=%d down=%d valid=%d target=%d",
				result.conn.ResolverLabel,
				result.conn.Domain,
				result.conn.UploadMTUBytes,
				result.conn.DownloadMTUBytes,
				len(validConns),
				target,
			)
		}
		if len(validConns) >= target {
			...
			if len(validConns) < len(scanConnections) {
				if c.log != nil {
					c.log.Infof(
				...
				c.debugf(
					"early-start threshold reached: target=%d selected=%s remaining_probe_count_at_least=%d",
					target,
					debugConnectionSummary(validConns, 8),
					max(0, len(scanConnections)-len(validConns)),
				)
...
func (c *Client) startMTUProbeScan(ctx context.Context, scanConnections []Connection, uploadCaps map[string]int, workerCount int, counters *mtuScanCounters) <-chan mtuScanResult {
	total := len(scanConnections)
	...
	if workerCount <= 1 {
		go func() {
			...
			for idx := range scanConnections {
				...
				conn := scanConnections[idx]
				scanned := c.runConnectionMTUTestDetached(ctx, conn, idx+1, total, uploadCaps[conn.Domain], counters)
				select {
				case results <- mtuScanResult{index: idx, conn: scanned}:
				case <-ctx.Done():
					return
				}
			}
...
func mtuJobBufferSize(workerCount int, total int) int {
	if total <= 0 {
		return 0
	}
	size := max(1, workerCount*2)
	if size > total {
		return total
	}
	return size
...
func (c *Client) initialMTUEarlyStartTarget(total int) int {
	...
	target := c.cfg.PacketDuplicationCount
	if c.cfg.SetupPacketDuplicationCount > target {
		target = c.cfg.SetupPacketDuplicationCount
	}
	if target < 3 {
		target = 3
	}
...
func (c *Client) activateInitialMTUSelection(validConns []Connection) error {
	selectedConns, minUpload, minDownload, minUploadChars := c.optimizeMTUResolvers(validConns)
	if len(selectedConns) == 0 {
		return ErrNoValidConnections
	}
	c.debugf(
		"activating initial MTU selection: candidates=%d selected=%s synced_upload=%d synced_download=%d upload_chars=%d",
		len(validConns),
		debugConnectionSummary(selectedConns, 8),
		minUpload,
		minDownload,
		minUploadChars,
	)
	selected := make(map[string]struct{}, len(selectedConns))
	for _, conn := range selectedConns {
		selected[conn.Key] = struct{}{}
	...
	for _, conn := range validConns {
		if _, ok := selected[conn.Key]; !ok {
			c.storeInactiveMTUConnection(conn)
		}
	}

	c.applySyncedMTUState(minUpload, minDownload, minUploadChars)
	c.appendMTUUsageSeparatorOnce()
	c.logConnectionProgress("selecting", 85, "valid", len(selectedConns))
...
func (c *Client) finishMTUScanInBackground(results <-chan mtuScanResult) {
	...
	invalid := 0
	lowMTU := 0
	c.debugf("background MTU scan started")
	for result := range results {
		processed++
		conn := result.conn
		...
		if activate {
			...
			c.debugf(
				"background MTU resolver activated: resolver=%s domain=%s up=%d down=%d",
				conn.ResolverLabel,
				conn.Domain,
				conn.UploadMTUBytes,
				conn.DownloadMTUBytes,
			...
			if !conn.IsValid {
				...
				if conn.UploadMTUBytes < mtuState.UploadMTU || conn.DownloadMTUBytes < mtuState.DownloadMTU {
					lowMTU++
				}
			}
			c.debugf(
				"background MTU resolver kept inactive: resolver=%s domain=%s valid=%t up=%d down=%d synced=%d/%d",
				conn.ResolverLabel,
				conn.Domain,
				conn.IsValid,
				conn.UploadMTUBytes,
				conn.DownloadMTUBytes,
				c.mtuStateSnapshot().UploadMTU,
				c.mtuStateSnapshot().DownloadMTU,
			)
			c.storeInactiveMTUConnection(conn)
		}
	}
	...
	c.debugf(
		"background MTU scan finished: processed=%d activated=%d inactive=%d invalid=%d low_mtu=%d active_now=%d",
		processed,
		activated,
		inactive,
		invalid,
...
func (c *Client) shouldActivateBackgroundMTUConnection(conn Connection) bool {
	...
	return conn.UploadMTUBytes >= mtuState.UploadMTU && conn.DownloadMTUBytes >= mtuState.DownloadMTU
}
...
func (c *Client) storeInactiveMTUConnection(conn Connection) {
	if c == nil || c.balancer == nil || conn.Key == "" {
		return
	}
	uploadChars := conn.UploadMTUChars
	if uploadChars <= 0 && conn.UploadMTUBytes > 0 {
		uploadChars = c.encodedCharsForPayload(conn.UploadMTUBytes)
	}
	_ = c.balancer.ApplyMTUProbeResult(
		conn.Key,
...
func (c *Client) recheckInactiveResolver(ctx context.Context, conn Connection) {
	...
	mtuState := c.mtuStateSnapshot()
	conn.UploadMTUBytes = mtuState.UploadMTU
	conn.UploadMTUChars = c.encodedCharsForPayload(mtuState.UploadMTU)
	conn.DownloadMTUBytes = mtuState.DownloadMTU
...
func (c *Client) recheckResolverUploadMTU(ctx context.Context, conn Connection, transport *udpQueryTransport) bool {
	...
	for attempt := 0; attempt < c.mtuTestRetries; attempt++ {
		...
		mtuState := c.mtuStateSnapshot()
		passed, _, err := c.sendUploadMTUProbe(ctx, conn, transport, mtuState.UploadMTU, timeout, mtuProbeOptions{
			Quiet:   true,
			IsRetry: attempt > 0,
		})
		if err == nil && passed {
...
func (c *Client) recheckResolverDownloadMTU(ctx context.Context, conn Connection, transport *udpQueryTransport) bool {
	...
	for attempt := 0; attempt < c.mtuTestRetries; attempt++ {
		...
		mtuState := c.mtuStateSnapshot()
		passed, _, err := c.sendDownloadMTUProbe(ctx, conn, transport, mtuState.DownloadMTU, mtuState.UploadMTU, timeout, mtuProbeOptions{
			Quiet:   true,
			IsRetry: attempt > 0,
		})
...
func (c *Client) confirmResolverDown(conn *Connection, window time.Duration) bool {
	...
	mtuSize := c.mtuStateSnapshot().UploadMTU
	if mtuSize < 32 {
		mtuSize = 32
	}
...
func (c *Client) runConnectionMTUTestDetached(ctx context.Context, conn Connection, serverID int, total int, maxUploadPayload int, counters *mtuScanCounters) (resultConn Connection) {
	resultConn = conn
	if conn.Key == "" {
		return resultConn
	}
	...
	resultConn.UploadMTUBytes = decision.uploadBytes
	resultConn.UploadMTUChars = decision.uploadChars
	resultConn.DownloadMTUBytes = decision.downloadBytes
	resultConn.MTUResolveTime = decision.resolveTime
	resultConn.IsValid = decision.active
	...
	c.recordScannerReportValid(resultConn)
	c.appendMTUSuccessLine(&resultConn)
	c.markResolverValid(resultConn.Key)
	c.logMTUProgress(counters, total)
	return resultConn
}
...
func (c *Client) runConnectionMTUTest(ctx context.Context, conn Connection, serverID int, total int, maxUploadPayload int, counters *mtuScanCounters) {
	...
	if acceptedConn, ok := c.balancer.GetConnectionByKey(conn.Key); ok {
		c.recordScannerReportValid(acceptedConn)
...
func (c *Client) runMTUProbe(ctx context.Context, conn Connection, maxUploadPayload int) (mtuConnectionProbeResult, mtuRejectReason) {
	if c != nil && c.mtuProbeFunc != nil {
		return c.mtuProbeFunc(ctx, conn, maxUploadPayload)
	}
	return c.probeConnectionMTU(ctx, conn, maxUploadPayload)
}
