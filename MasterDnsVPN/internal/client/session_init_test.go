...
func TestNextSessionInitAttemptPrefersFastPostMTUCandidate(t *testing.T) {
	c := buildTestClientWithResolvers(config.ClientConfig{}, "slow", "fast", "medium")
	c.applySyncedMTUState(120, 300, 180)
	c.balancer.ApplyMTUProbeResult("slow", 137, 200, 1000, 800*time.Millisecond, true)
	c.balancer.ApplyMTUProbeResult("fast", 137, 200, 1000, 100*time.Millisecond, true)
	c.balancer.ApplyMTUProbeResult("medium", 137, 200, 1000, 250*time.Millisecond, true)
	...
	if conn.Key != "fast" {
		t.Fatalf("expected fastest post-MTU resolver first, got %q", conn.Key)
	}

	conn, _, _, err = c.nextSessionInitAttempt()
	if err != nil {
		t.Fatalf("nextSessionInitAttempt returned error on second call: %v", err)
	}
	if conn.Key != "medium" {
		t.Fatalf("expected second fastest post-MTU resolver second, got %q", conn.Key)
	}
}
