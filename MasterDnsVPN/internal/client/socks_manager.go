...
func (c *Client) HandleSOCKS5(ctx context.Context, conn net.Conn) {
	...
	if c.socksRateLimit != nil {
		...
		if c.socksRateLimit.IsBlocked(ip) {
			c.debugf("local SOCKS connection rejected by rate limit: remote=%s", conn.RemoteAddr())
			_ = conn.Close()
			return
		}
	}
	...
	if _, err := io.ReadFull(conn, version); err != nil {
		c.debugf("local SOCKS connection closed before version read: remote=%s age=%s err=%v", conn.RemoteAddr(), time.Since(startedAt), err)
		_ = conn.Close()
		return
	}
	...
	switch version[0] {
	case SOCKS5_VERSION:
		c.debugf("local SOCKS5 handshake started: remote=%s", conn.RemoteAddr())
		c.handleSOCKS5Request(ctx, conn)
	case SOCKS4_VERSION:
		if !c.supportsSOCKS4() {
			c.debugf("local SOCKS4 rejected because auth policy does not allow it: remote=%s", conn.RemoteAddr())
			_ = conn.Close()
			return
		}
		c.debugf("local SOCKS4 handshake started: remote=%s", conn.RemoteAddr())
		c.handleSOCKS4Request(ctx, conn)
	default:
		c.debugf("local SOCKS connection rejected unsupported version: remote=%s version=%d", conn.RemoteAddr(), version[0])
		_ = conn.Close()
	}
}
...
func (c *Client) handleSOCKS5Request(ctx context.Context, conn net.Conn) {
	...
	if methodSelected == SOCKS5_AUTH_METHOD_NO_ACCEPTABLE {
		c.debugf("local SOCKS5 auth negotiation failed: remote=%s methods=%d auth_enabled=%t", conn.RemoteAddr(), len(methods), c.cfg.SOCKS5Auth)
		_ = conn.Close()
		return
	}
	...
	if cmd == SOCKS5_CMD_CONNECT {
		c.debugf("local SOCKS5 CONNECT request parsed: remote=%s target=%s:%d atyp=%d", conn.RemoteAddr(), addr, port, atyp)
		c.handleSOCKSConnect(ctx, conn, addr, port, atyp, SOCKS5_VERSION)
		return
	}

	if cmd == SOCKS5_CMD_UDP_ASSOCIATE {
		c.debugf("local SOCKS5 UDP ASSOCIATE request parsed: remote=%s target=%s:%d atyp=%d timeout=%s", conn.RemoteAddr(), addr, port, atyp, c.cfg.SOCKSUDPAssociateReadTimeout())
...
func (c *Client) handleSOCKSConnect(ctx context.Context, conn net.Conn, addr string, port uint16, atyp byte, socksVersion byte) {
	...
	uploadMTU := arqObj.MTU()
	if uploadMTU <= 0 {
		uploadMTU = c.mtuStateSnapshot().UploadMTU
	}
	fragments := fragmentPayload(targetPayload, uploadMTU)
	total := uint8(len(fragments))
	sn := uint16(0)
	c.debugf(
		"local SOCKS setup packet queued: stream=%d target=%s:%d socks_version=%d upload_mtu=%d fragments=%d setup_dup=%d age=%s",
		streamID,
		addr,
		port,
		socksVersion,
...
func (c *Client) handlePendingSOCKSLocalClose(streamID uint16, reason string) {
	...
	if s.StatusValue() != streamStatusSocksConnecting {
		c.debugf(
			"local SOCKS close ignored for non-pending stream: stream=%d status=%s reason=%s age=%s",
			streamID,
			s.StatusValue(),
			reason,
			time.Since(s.CreateTime),
		)
		s.socksResultMu.Unlock()
		return
	}
	s.SetStatus(streamStatusCancelled)
	c.debugf("local SOCKS pending connection cancelled: stream=%d reason=%s age=%s", streamID, reason, time.Since(s.CreateTime))
	if s.NetConn != nil {
		_ = s.NetConn.Close()
	}
...
func (c *Client) HandleSocksConnected(packet VpnProto.Packet) error {
	...
	s.socksResultMu.Unlock()
	if err != nil {
		c.debugf("local SOCKS success result write failed: stream=%d age=%s err=%v", packet.StreamID, time.Since(s.CreateTime), err)
	...
	c.debugf("local SOCKS connect completed: stream=%d age=%s", packet.StreamID, time.Since(s.CreateTime))
	return nil
}
...
func (c *Client) HandleSocksFailure(packet VpnProto.Packet) error {
	...
	s.socksResultMu.Unlock()
	if err != nil {
		c.debugf("local SOCKS failure result write failed: stream=%d packet=%s age=%s err=%v", packet.StreamID, Enums.PacketTypeName(packet.PacketType), time.Since(s.CreateTime), err)
		if errors.Is(err, errLateSocksResult) {
			if arqObj, arqErr := c.getStreamARQ(packet.StreamID); arqErr == nil {
	...
	c.debugf("local SOCKS connect failed: stream=%d packet=%s age=%s", packet.StreamID, Enums.PacketTypeName(packet.PacketType), time.Since(s.CreateTime))
