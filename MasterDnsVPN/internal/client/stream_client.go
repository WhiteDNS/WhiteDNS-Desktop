...
func (c *Client) new_stream(streamID uint16, conn net.Conn, targetPayload []byte) *Stream_client {
	...
	mtu := c.initialStreamUploadMTU(streamID)
	if mtu <= 0 {
	...
	c.debugf(
		"stream created: stream=%d status=%s initial_upload_mtu=%d has_local_conn=%t queue_cap=%d arq_window=%d",
		streamID,
		s.StatusValue(),
		mtu,
		conn != nil,
		c.cfg.EffectiveStreamQueueInitialCapacity(),
		c.cfg.ARQWindowSize,
	)
...
func (c *Client) initialStreamUploadMTU(streamID uint16) int {
	mtu := c.mtuStateSnapshot().UploadMTU
	if c == nil || c.balancer == nil || streamID == 0 {
		return mtu
	}

	c.balancer.EnsureStream(streamID)
	dupCount := c.runtimePacketDuplicationCount(Enums.PACKET_STREAM_DATA)
	conns, err := c.balancer.SelectTargetsForPayload(Enums.PACKET_STREAM_DATA, streamID, dupCount, 0)
	if err != nil || len(conns) == 0 {
		return mtu
	}
	if groupMTU := c.effectiveGroupUploadMTU(conns); groupMTU > 0 {
		c.debugf(
			"initial stream upload MTU selected from resolver group: stream=%d group_mtu=%d global_mtu=%d %s",
			streamID,
			groupMTU,
			mtu,
			debugConnectionSummary(conns, 6),
		)
		return groupMTU
	}
	return mtu
...
func (s *Stream_client) SetUploadMTU(mtu int) bool {
	if s == nil || mtu <= 0 {
		return false
	}
...
func (s *Stream_client) CapUploadMTU(mtu int) bool {
	if s == nil || mtu <= 0 {
		return false
	}
	arqObj, ok := s.Stream.(*arq.ARQ)
