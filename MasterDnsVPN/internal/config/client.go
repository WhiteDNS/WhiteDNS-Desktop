...
type ClientConfig struct {
	...
	MTUStartupLossVerifyEnabled           bool              `toml:"MTU_STARTUP_LOSS_VERIFY_ENABLED"`
	MTUStartupLossVerifySamples           int               `toml:"MTU_STARTUP_LOSS_VERIFY_SAMPLES"`
	MTUStartupLossVerifyMaxLossPercent    int               `toml:"MTU_STARTUP_LOSS_VERIFY_MAX_LOSS_PERCENT"`
	MTUStartupLossVerifyCandidates        int               `toml:"MTU_STARTUP_LOSS_VERIFY_CANDIDATES"`
	MTURecheckEnabled                     bool              `toml:"MTU_RECHECK_ENABLED"`
	MTURecheckIntervalMinutes             int               `toml:"MTU_RECHECK_INTERVAL_MINUTES"`
...
func defaultClientConfig() ClientConfig {
	return ClientConfig{
		...
		MTUStartupLossVerifyMaxLossPercent:    34,
...
func finalizeClientConfig(cfg ClientConfig) (ClientConfig, error) {
	...
	cfg.PacketDuplicationCount = clampInt(defaultIntBelow(cfg.PacketDuplicationCount, 1, 1), 1, 10)
	cfg.SetupPacketDuplicationCount = clampInt(defaultIntBelow(cfg.SetupPacketDuplicationCount, 1, max(3, cfg.PacketDuplicationCount)), cfg.PacketDuplicationCount, 12)
	...
	if cfg.MTUStartupLossVerifySamples < 0 {
		return cfg, fmt.Errorf("MTU_STARTUP_LOSS_VERIFY_SAMPLES cannot be negative")
	}
	if cfg.MTUStartupLossVerifyCandidates < 0 {
		return cfg, fmt.Errorf("MTU_STARTUP_LOSS_VERIFY_CANDIDATES cannot be negative")
	}
	if cfg.MTUStartupLossVerifyMaxLossPercent < 0 || cfg.MTUStartupLossVerifyMaxLossPercent > 100 {
		return cfg, fmt.Errorf("invalid MTU_STARTUP_LOSS_VERIFY_MAX_LOSS_PERCENT: %d", cfg.MTUStartupLossVerifyMaxLossPercent)
	}
	if cfg.MTURecheckIntervalMinutes < 0 {
		return cfg, fmt.Errorf("MTU_RECHECK_INTERVAL_MINUTES cannot be negative")
	}
	cfg.MTUStartupLossVerifySamples = clampInt(defaultIntBelow(cfg.MTUStartupLossVerifySamples, 1, 3), 1, 30)
	cfg.MTUStartupLossVerifyCandidates = clampInt(defaultIntBelow(cfg.MTUStartupLossVerifyCandidates, 1, 3), 1, 16)
