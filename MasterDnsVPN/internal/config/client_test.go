...
`), 0o644); err != nil {
	...
	if cfg.MTUStartupLossVerifyEnabled || cfg.MTUStartupLossVerifySamples != 3 || cfg.MTUStartupLossVerifyMaxLossPercent != 34 || cfg.MTUStartupLossVerifyCandidates != 3 {
		t.Fatalf("unexpected startup loss defaults: enabled=%v samples=%d maxLoss=%d candidates=%d",
	...
	if cfg.MTURecheckEnabled || cfg.MTURecheckIntervalMinutes != 5 {
		t.Fatalf("unexpected mtu recheck defaults: enabled=%v interval=%d", cfg.MTURecheckEnabled, cfg.MTURecheckIntervalMinutes)
	}
...
func TestLoadClientConfigPreservesMTURecheckAndStartupLossSettings(t *testing.T) {
	dir := t.TempDir()
...
MTU_STARTUP_LOSS_VERIFY_SAMPLES = 5
MTU_STARTUP_LOSS_VERIFY_MAX_LOSS_PERCENT = 12
MTU_STARTUP_LOSS_VERIFY_CANDIDATES = 4
MTU_RECHECK_ENABLED = false
MTU_RECHECK_INTERVAL_MINUTES = 0
`), 0o644); err != nil {
		t.Fatalf("WriteFile config failed: %v", err)
	}
	if err := os.WriteFile(resolversPath, []byte("8.8.8.8\n"), 0o644); err != nil {
	...
	if cfg.MTUStartupLossVerifyEnabled || cfg.MTUStartupLossVerifySamples != 5 || cfg.MTUStartupLossVerifyMaxLossPercent != 12 || cfg.MTUStartupLossVerifyCandidates != 4 {
		t.Fatalf("unexpected startup loss settings: %#v", cfg)
	}
	if cfg.MTURecheckEnabled || cfg.MTURecheckIntervalMinutes != 0 {
		t.Fatalf("unexpected recheck settings: enabled=%v interval=%d", cfg.MTURecheckEnabled, cfg.MTURecheckIntervalMinutes)
	}
}

func TestLoadClientConfigRejectsNegativeMTURecheckInterval(t *testing.T) {
	dir := t.TempDir()
...
DATA_ENCRYPTION_METHOD = 1
ENCRYPTION_KEY = "secret"
MTU_RECHECK_INTERVAL_MINUTES = -1
`), 0o644); err != nil {
		t.Fatalf("WriteFile config failed: %v", err)
	}
	if err := os.WriteFile(resolversPath, []byte("8.8.8.8\n"), 0o644); err != nil {
	...
	if _, err := LoadClientConfig(configPath); err == nil {
		t.Fatal("LoadClientConfig should reject negative MTU_RECHECK_INTERVAL_MINUTES")
	}
}
