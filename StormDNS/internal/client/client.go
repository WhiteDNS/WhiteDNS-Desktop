...
type Client struct {
	...
	resolverClassificationMu sync.RWMutex
	resolverClassifications  map[string]resolverClassification
...
func New(cfg config.ClientConfig, log *logger.Logger, codec *security.Codec) *Client {
	...
	c := &Client{
		...
		resolverClassifications:               make(map[string]resolverClassification),
