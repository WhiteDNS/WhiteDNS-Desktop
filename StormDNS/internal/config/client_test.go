...
DATA_ENCRYPTION_METHOD = 1
ENCRYPTION_KEY = "secret"
UPLOAD_PACKET_DUPLICATION_COUNT = 90
DOWNLOAD_PACKET_DUPLICATION_COUNT = 90
`), 0o644); err != nil {
		t.Fatalf("WriteFile config failed: %v", err)
	}
	if err := os.WriteFile(resolversPath, []byte("8.8.8.8\n"), 0o644); err != nil {
	...
	if cfg.UploadPacketDuplicationCount != 90 {
		t.Fatalf("unexpected upload duplication: got=%d want=90", cfg.UploadPacketDuplicationCount)
	}
	if cfg.DownloadPacketDuplicationCount != 90 {
		t.Fatalf("unexpected download duplication: got=%d want=90", cfg.DownloadPacketDuplicationCount)
	}
	if cfg.UploadSetupPacketDuplicationCount != 90 {
		t.Fatalf("unexpected upload setup duplication: got=%d want=90", cfg.UploadSetupPacketDuplicationCount)
	}
	if cfg.DownloadSetupPacketDuplicationCount != 90 {
		t.Fatalf("unexpected download setup duplication: got=%d want=90", cfg.DownloadSetupPacketDuplicationCount)
	}
}
